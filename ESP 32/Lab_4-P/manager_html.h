...
<div class="wrap">
  ...
  <p style="opacity:.7;margin-top:10px;">AP IP: <span id="apip">{{AP_IP}}</span></p>
</div>
<script>
...
$('save').onclick=async()=>{
  ...
  if(j && j.ok){
    if(j.ip){
      $('msg').innerHTML = `เชื่อมต่อสำเร็จ IP ใหม่: <b>${j.ip}</b> · <a href="http://${j.ip}/home">เปิดหน้า</a>`;
      $('apip').textContent = j.ip;
    }else{
      $('msg').textContent = 'บันทึกแล้ว กำลังเปลี่ยนโหมด...';
    }
    if(j.redirect){
      setTimeout(()=>{ location.href = j.redirect; }, 1200);
    }
  }else{
    $('msg').textContent = t.includes('ssid required') ? 'กรุณากรอก SSID' : (t || 'บันทึกไม่สำเร็จ');
  }
};
</script>
</body></html>)HTML";

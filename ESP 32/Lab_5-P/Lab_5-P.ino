...
void setup() {
  ...
  server.on("/save", HTTP_POST, [](){
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      blynkBegin();
      String resp = String("{\"ok\":true,\"redirect\":\"http://") + WiFi.localIP().toString() + "/home\"}";
      server.send(200, "application/json; charset=utf-8", resp);
    } else {
      startPortal();
      server.send(200, "application/json; charset=utf-8", "{\"ok\":false,\"msg\":\"connect failed\"}");
    }
  });
